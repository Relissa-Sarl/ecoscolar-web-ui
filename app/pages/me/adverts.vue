        ...
        <div class="mt-8 p-6">
            <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-wrap items-center justify-center gap-3 ">
                    ...
                    <NuxtLink :to="localePath('/adverts/create-advert')"" class="ml-auto rounded-xl bg-[#105B53] px-4 py-2 text-sm font-medium text-white hover:bg-[#0c4540] dark:text-gray-300">
                        {{ $t('me.adverts.add') }}
                    </NuxtLink>
                </div>
                <!-- Adverts -->
                    ...
                    <div v-else v-for="advert in adverts" :key="advert.id" v-show="filterByType(advert)">
                        <div class="group h-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-lg dark:border-gray-800 dark:bg-gray-950">
                            ...
                            <div class="flex gap-3 border-t border-gray-200 px-5 py-3 dark:border-gray-800">
                                ...
                                <button aria-label="{{ $t('me.adverts.delete') }}" :class="[isActionDisabled(advert.status) ? 'cursor-not-allowed rounded-lg border-gray-500 text-gray-500' : 'rounded-lg border border-red-500 p-2 text-red-500 transition-colors hover:bg-red-500/10']" @click="!isActionDisabled(advert.status) ? deleteAdvert(advert.id) : null">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 material-symbols-outlined text-base">
                    ...
                    <div v-show="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                        <div class="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
                            <h3 class="mb-4 text-lg font-semibold">{{ $t('me.adverts.deleteConfirmTitle') }}</h3>
                            <p class="mb-6 text-sm text-gray-600 dark:text-gray-400">{{ $t('me.adverts.deleteConfirmMessage') }}</p>
                            <div class="flex justify-end gap-4">
                                <button @click="showDeleteConfirm = false; advertToDelete = null" class="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">
                                    {{ $t('me.adverts.cancel') }}
                                </button>
                                <button @click="confirmDelete" class="rounded-lg bg-red-500 px-4 py-2 text-sm text-white hover:bg-red-600">
